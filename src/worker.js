...
onmessage = function(e) {
    const { pixels, width, height, isEncrypt } = e.data;
    ...
    for (let i = 0; i < total; i++) {
        ...
        if (isEncrypt) {
            res[y2 * width + x2] = pixels[y1 * width + x1];
        } else {
            res[y1 * width + x1] = pixels[y2 * width + x2];
        }
    }
    postMessage({ buffer: res.buffer }, [res.buffer]);
};